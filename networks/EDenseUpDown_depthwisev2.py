...
from .spl_loss import CPLoss


class one_conv(nn.Module):
    def __init__(self, input_feature, compress, group=8):
        super(one_conv, self).__init__()
        ## todo: the affection of LCL
        self.group_dim = input_feature // group

        self.compress_skip = nn.Conv2d(in_channels = input_feature, out_channels = compress, kernel_size=1, stride=1, padding=0)

        # self.group_conv = nn.ModuleList([
        #     nn.Conv2d(in_channels=input_feature//group, out_channels=self.group_dim, kernel_size=3, stride=1,
        #               padding=1) for _ in range(group)
        # ])
        self.group_conv = nn.Conv2d(in_channels=input_feature, out_channels=input_feature, kernel_size=3,
                                    stride=1, padding=1, groups=group)
        self.compress_group = nn.Conv2d(input_feature, input_feature-compress, kernel_size=1, stride=1, padding=0)

    def forward(self, x):
        out = self.compress_skip(x)

        # x = torch.split(x, self.group_dim, dim=1)
        #
        # media = [conv(group_in) for group_in, conv in zip(x, self.group_conv)]
        # media = torch.cat(media, dim=1)
        media = self.group_conv(x)
        media = self.compress_group(media)
...
class Net(nn.Module):
    def __init__(self, opt):
        ...
        self.conv_output_list = nn.Conv2d(in_channels=num_feature, out_channels=inChannels, kernel_size=3, stride=1,
                      padding=1)


        self.up_list =  Up(self.scale, num_feature, num_feature)
        self.down_list = Down(self.scale, num_feature, num_feature)


        self.IRU_body_list =  self._make_iru_body(num_feature, compress, layers, num_blocks)
    ...
    def forward(self, x):
        ...
        high_fe = self.IRU_body_list(init_fe) + init_fe
        up_fe = self.up_list(high_fe)
        out.append(self.conv_output_list(up_fe))

        # output-2
        for idx in range(1,self.iteration):
            down_fe = self.down_list(up_fe)
            res_fe = high_fe-down_fe
            high_fe = self.IRU_body_list(res_fe)+down_fe
            up_fe = self.up_list(high_fe)
            out.append(self.conv_output_list(up_fe) + out[idx-1])
